...
def createHierarchicalAttentionModel(maxSeq,
    ...
    label_file='G:\HTMC-2\DATA\processed_data\kaggle\data29633_digital_label/label_2_vector'
...
if __name__ == '__main__':
    ...
    for train, test in ks:
        ...
        filepath = "G:\HTMC-2\model\HE_HMC\KAGGLE/gru_layer3_fold"+str(i)+".h5"
        checkpoint = ModelCheckpoint(filepath, monitor='val_acc', verbose=1, save_best_only=True)
        mode=createHierarchicalAttentionModel(300,embWeights=pretrained_w2v)
        mode.fit([data[train],label2[train],label2[train]],[y_train],batch_size=256,epochs=100,validation_data=([data[test],label2[test],label2[test]],[y_test]),callbacks=[checkpoint,early_stopping])



        #获取预测标签
        mode = keras.models.load_model(filepath)  # 加载模型
        score = mode.evaluate([data[test],label2[test],label2[test]], [y_test], batch_size=256)
        scores.append(score[1])
        predict=mode.predict([data[test],label2[test],label2[test]],batch_size=256)
        predict = np.argmax(predict, axis=1)
        label_save_name = r'G:\HTMC-2\DATA\processed_data\kaggle\data29633_digital_label\predict_label/layer3_fold'+str(i)+'_predict'
