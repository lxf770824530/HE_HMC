...
if __name__ == '__main__':
    filepath = "../DATA/WOS5736/gru_layer2.h5"
    checkpoint = ModelCheckpoint(filepath, monitor='val_acc', verbose=1, save_best_only=True)
    lrate = LearningRateScheduler(step_decay)
    pretrained_w2v, _, _ = pl.load(open('../DATA/WOS5736/emb_matrix_glove_300', 'rb'))
    print(np.shape(pretrained_w2v))
    # train_txt,y1,y2,y3=pl.load(open('../DATA/WOS11967/train_txt-len-y_300_pad0_glove','rb'))
    # test_txt, ty1, ty2, ty3 = pl.load(open('../DATA/WOS11967/test_txt-len-y_300_pad0_glove', 'rb'))
    ...
    mode.fit([train_txt],[y2],batch_size=64,epochs=150,validation_data=([test_txt,],[ty2]),callbacks=[checkpoint])
