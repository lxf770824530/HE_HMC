...
def createHierarchicalAttentionModel(maxSeq,
    ...
    labels_ge_vector = pl.load(open(r'G:\HTMC-2\DATA\processed_data\bestbuy\bestbuy_digital_label/ge_label1_2', 'rb'))
...
if __name__ == '__main__':
    filepath = r"G:\HTMC-2\model\HE_HMC\ablation_study\graph_Embedding\bestbuy/gru_layer2_ge.h5"
    ...
    train_txt, y1, y2 = pl.load(open(r'G:\HTMC-2\DATA\processed_data\bestbuy\bestbuy_digital_label/train_txt-len-y_300_pad0_glove', 'rb'))      #3层
    ...
    # train_txt, y1, y2= pl.load(open('../DATA/patent11072/train_txt-len-y_300_pad0_glove', 'rb'))
    # test_txt, ty1, ty2 = pl.load(open('../DATA/patent11072/test_txt-len-y_300_pad0_glove', 'rb'))
    ...
    mode.fit([train_txt,y1],[y2],batch_size=64,epochs=150,validation_data=([test_txt,ty1],[ty2]),callbacks=[checkpoint,early_stopping])

    mode = keras.models.load_model(filepath)  # 加载模型
    score = mode.evaluate([test_txt,ty1],[ty2], batch_size=256)
    print(score)
    predict = mode.predict([test_txt,ty1], batch_size=256)
    predict = np.argmax(predict, axis=1)
    label_save_name = r'G:\HTMC-2\model\HE_HMC\ablation_study\graph_Embedding\bestbuy\predict_label/layer2_predict'
