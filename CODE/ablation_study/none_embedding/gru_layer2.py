...
if __name__ == '__main__':
    ...
    filepath = r"G:\HTMC-2\model\HE_HMC\ablation_study\webservice/gru_layer2.h5"
    checkpoint = ModelCheckpoint(filepath, monitor='val_acc', verbose=1, save_best_only=True)
    lrate = LearningRateScheduler(step_decay)
    pretrained_w2v, _, _ = pl.load(open(r'G:\HTMC-2\DATA\processed_data\webservice\data/emb_matrix_glove_300', 'rb'))
    print(np.shape(pretrained_w2v))
    # train_txt, y1, y2, y3= pl.load(open(r'G:\HTMC-2\DATA\processed_data\bestbuy\bestbuy_digital_label/train_txt-len-y_300_pad0_glove', 'rb'))
    # test_txt, ty1, ty2,ty3= pl.load(open(r'G:\HTMC-2\DATA\processed_data\bestbuy\bestbuy_digital_label/test_txt-len-y_300_pad0_glove', 'rb'))
    train_txt, y1, y2= pl.load(open(r'G:\HTMC-2\DATA\processed_data\webservice\data/train_txt-len-y_300_pad0_glove', 'rb'))
    test_txt, ty1, ty2= pl.load(open(r'G:\HTMC-2\DATA\processed_data\webservice\data/test_txt-len-y_300_pad0_glove', 'rb'))
    y2=to_categorical(y2,class_num)
    ty2=to_categorical(ty2,class_num)                                                                           #类别数调整
    ...
    mode.fit([train_txt],[y2],batch_size=64,epochs=150,validation_data=([test_txt,],[ty2]),callbacks=[checkpoint,early_stopping])

    mode = keras.models.load_model(filepath)  # 加载模型
    score = mode.evaluate([test_txt,],[ty2], batch_size=256)
    print(score)
    predict = mode.predict([test_txt,], batch_size=256)
    predict = np.argmax(predict, axis=1)
    label_save_name = r'G:\HTMC-2\model\HE_HMC\ablation_study\webservice\predict_label/layer2_predict'
