...
def createHierarchicalAttentionModel(maxSeq,
    ...
    labels_vector = pl.load(open('../DATA/dbpedia/label_1_vector', 'rb'))                                                       #修改类别嵌入路径------
    ...
    labels_ge_vector = pl.load(open('../DATA/dbpedia/dbpedia_label1-2', 'rb'))                                                  #修改图嵌入路径---------
...
if __name__ == '__main__':
    ...
    mode.fit([train_txt,y1,y1],[y2],batch_size=256,epochs=1,validation_data=([test_txt,ty1,ty1],[ty2]),callbacks=[checkpoint])
    #真实标签+文本
